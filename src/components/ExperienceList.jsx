...
const ExperienceList = ({ language = "es" }) => {
  ...
  return (
    <div className="relative pl-6 space-y-12 text-[#131313]">
      {/* Editorial Timeline Line */}
      <div className="absolute top-2 bottom-0 left-0 w-px bg-[#131313] opacity-20"></div>

      {dataToShow.map((exp, index) => (
        <div key={exp.id || `${exp.cargo}-${exp.empresa}-${index}`} className="relative flex flex-col md:flex-row md:justify-between md:items-start gap-2">
          {/* Editorial Square Bullet */}
          <div className="absolute -left-[27px] top-1.5 w-3 h-3 rounded-full bg-[#1398ff]"></div>
          ...
          <div className="md:w-1/4 md:text-right mt-2 md:mt-0">
            <span className="font-mono text-[10px] uppercase font-bold text-[#131313]/80 block leading-tight">
